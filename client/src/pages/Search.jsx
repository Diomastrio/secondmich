...
export default function Search() {
  ...
  const handleChange = (e) => {
    if (e.target.id === "término de búsqueda") {
      setSidebarData({ ...sidebarData, searchTerm: e.target.value });
    }
    if (e.target.id === "clasificar") {
      const order = e.target.value || "desc";
      setSidebarData({ ...sidebarData, sort: order });
    }
    if (e.target.id === "categoría") {
      const category = e.target.value || "sin categoría";
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const urlParams = new URLSearchParams(location.search);
    urlParams.set("término de búsqueda", sidebarData.searchTerm);
    urlParams.set("clasificar", sidebarData.sort);
    urlParams.set("categoría", sidebarData.category);
    const searchQuery = urlParams.toString();
